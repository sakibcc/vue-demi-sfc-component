  ...
  create() {
    ...
    this.insertAdjacentHTML(
      ...
      `<div style="width: ${this.getWidth()}">
           ...
           <ui-select-list id="${strId}" role="listbox" aria-expanded="false" class="${DATALIST_CLASS}" ${
        !this.multiple ? 'aria-hidden="true"' : ''
      } data-custom-scroll="${isCustomScroll}"></ui-select-list>
        </div>`
    )
        ...
        const funStep = (arrData, arrIndex) => {
          ...
          arrData.forEach((objData, numIndex) => {
            ...
            if (objData[DISABLED] != true) {
              ...
              if (objData.data) {
                strHtmlStep +=
                  '<div class="' + CL.add('xx') + '"><div class="' + CL.add('x') + '" role="listbox">' + funStep(objData.data, arrCurrentIndex) + '</div></div>'
              }
            } else {
          ...
          arrButtons = [
            {
              ...
              events: () => {
                eleTarget.dispatchEvent(
                  new CustomEvent('ensure', {
                    detail: {
                      drop: this
                    }
                  })
...
const autoInitAndWatchingIsDropAttr = () => {
  ...
  const observer = new MutationObserver((mutationsList) => {
    mutationsList.forEach((mutation) => {
      mutation.addedNodes &&
        mutation.addedNodes.forEach((eleAdd) => {
          if (!eleAdd.tagName) {
            return
          }
 ...
 * @param options { symbol, onShow, onHide }
 */
...
class Color extends HTMLInputElement {
  ...
  create() {
    ...
    const strHtmlCurrent = `<div class="${Color.addClass('current')}">
    ...
    const strHtmlBody =
      ...
      (function () {
        ...
        // color left
        strHtml +=
          `<aside class="${Color.addClass('basic', 'l')}">` +
          (function () {
            return arrFixedColor
              ...
              .map(function (color) {
                ...
                return `<a href="javascript:" class="${Color.addClass(
                  'lump'
        ...
        strHtml =
          strHtml +
          `<div class="${Color.addClass('basic', 'r')}">` +
          (function () {
            let strHtmlRG = ''

            arrBasicColorPreset.forEach((colorItem) => {
              strHtmlRG += `<a href="javascript:" title="#${colorItem}${isSupportOpacity ? 'ff' : ''}" class="${Color.addClass(
                'lump',
                'preset'
      ...
      (function () {
        ...
        html += `<div class="${Color.addClass('more', 'l')}">
                <a href="javascript:" class="${Color.addClass('cover', 'white')}" aria-label="色域背景块" role="region"></a><div class="${Color.addClass(
          'circle'
        )} colorCircle"></div>
                <div class="${Color.addClass('gradient')}">
                    ...
                    <div class="${Color.addClass('more', 'fill')} colorFill">
                        ...
                        <div class="${Color.addClass('gradient')}" style="background: linear-gradient(#ffffff 0%, rgba(255,255,255,0) 50%, rgba(0,0,0,0) 50%, ${
          Color.defaultValue
        } 100%);">
                        </div>
                    </div>
            ...
            if (eleClose) {
              eleClose.addEventListener('click', () => {
                ...
                if (
                  window.isKeyEvent &&
                  eleTargetElement &&
                  eleActiveElement !== eleClose &&
                  document.querySelector(
                    'a[data-target="' + attrActiveElement + '"],input[data-target="' + attrActiveElement + '"],button[data-target="' + attrActiveElement + '"]'
                  ) &&
  ...
  class Datalist extends HTMLInputElement {
    ...
    refresh(data) {
      ...
      if (arrData && arrData.length) {
        ...
        arrData.forEach((objData, numIndex) => {
          ...
          let strLabelStrip = Datalist.stripHTML(objData.label || '')
            .trim()
            .replace(/\n/g, '')
          ...
          if (objData.label) {
            ...
            strHtmlList = `${strHtmlList}<li class="${CL.add(
              'option'
            )}${strClassList}" data-value="${strValueStrip}" label="${strLabelStrip}" data-index="${numIndex}"><label class="${CL.add('label')}">${
              objData.label
            }</label><span class="${CL.add('value')}">${strValueEncode}</span></li>`
          } else {
            // 但是，一些属性值，还是遵循HTML规范
            strHtmlList = `${strHtmlList}<li class="${CL.add(
              'option'
            )}${strClassList}" data-value="${strValueStrip}" data-index="${numIndex}"><span class="${CL.add('value')}">${strValueEncode}</span></li>`
          }
        })
      }
    ...
    create() {
      ...
      if (!this.element.target) {
        ...
        // 方便区分不同的数据列表
        if (this.id) {
          eleTarget.classList.add(CL.add(this.id.replace(/[A-Z]/g, (matches) => `-${matches.toLowerCase()}`).replace(/^-+|-+$/g, '')))
        }
        // 载入页面
        document.body.appendChild(eleTarget)
        ...
        switch (event.code) {
          ...
          case 'ArrowDown': {
            ...
            if (this.display == true && arrData && arrData.length) {
              ...
              // 然后索引往后挪一位
              let numIndexFilterMatch = arrIndexMatchAble.indexOf(numIndex)
          ...
          case 'Delete': {
            ...
            if (this.display == true && this.params.twice == true && eleSelected) {
              let strValueSelected = eleSelected.getAttribute('data-value')
              // 清除本地存储内容
              this.removeStore(strValueSelected)
              // data中对应对象删除
              arrData = arrData.filter((objData) => objData.value != strValueSelected)
              // 阻止默认删除行为
              event.preventDefault()
    ...
    show() {
      ...
      if (numWidthTarget == 'auto') {
        ...
        numWidthTarget = numWidthTarget.call(this, eleTarget)
      }
...
const DateTime = (() => {
  ...
  class Component extends HTMLInputElement {
    ...
    date() {
      ...
      // 头部月份公用结束
      strHtml = `${strHtml}<a href="javascript:" class="${CL.date('switch')}" data-type="month" role="button" aria-label="快速切换月份">${arrDate
        .slice(0, 2)
        .join('-')}</a></div>`
    ...
    ['date-range']() {
      ...
      // 今月月份显示
      strHtml = `${strHtml}<span class="${CL.date('switch')}">${new Date(arrDate[0], numPrevMonth, arrDate[2])
        .toArray()
        .slice(0, 2)
        .join('-')}</span></div><div class="${CL.date('half')}">`
      ...
      if (this.datetimeformat != 'H:mm') {
        strHtml =
          ...
          `<div class="${CL.time('col')}" data-type="ampm">
                    ${arrAmpm
                      .map((obj, index) => `<button class="${CL.time('item')}${obj.selected ? ' selected' : ''}" data-index="${index}">${obj.value}</button>`)
                      .join('')}
                </div>`
      }
      // 时分秒
      strHtml =
        ...
        `<div class="${CL.time('col')}" data-type="hour">
                ${arrHours
                  .map(
                    (obj, index) =>
                      `<button class="${CL.time('item')}${obj.selected ? ' ' + SELECTED : ''}" data-index="${index}"${
                        obj.disabled ? ' disabled' : ''
                      } data-visibility="${obj.visibility}">${obj.value}</button>`
                  )
                  .join('')}
            </div>`
      strHtml =
  ...
  document.validate = (function () {
    return {
      ...
      checkValidity(element) {
        ...
        if (element.length) {
          return [].slice.call(element).every(
            function (ele) {
...
HTMLElement.prototype.pagination = function (options) {
  ...
  const { total = 0, current = 1, per = 15, href = null, loading = false } = this.dataset
  ...
  if (!''.interpolate) {
    ...
    String.prototype.interpolate = function (params) {
      ...
      return new Function(
        ...names,
        `return \`${(function (str) {
          return str.replace(/&(lt|gt|amp|quot);/gi, function (all, t) {
            return {
              lt: '<',
              gt: '>',
              amp: '&',
...
class Form extends HTMLFormElement {
  ...
  connectedCallback() {
    // 表单提交按钮元素的获取
    let eleSubmit =
      [...this.elements].filter(function (control) {
